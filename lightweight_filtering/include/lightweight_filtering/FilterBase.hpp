...
public:
	...
	void addUpdateMeas(const typename std::tuple_element<i,decltype(mUpdates_)>::type::mtMeas& meas, double t)
	{
		...
		if(t<= frontWarningTime_) 
			gotFrontWarning_ = true;
	...
	void updateSafe(const double* maxTime = nullptr)
	{
		...
		if(maxTime != nullptr && nextSafeTime > *maxTime) 
			nextSafeTime = *maxTime;
	...
	void update(mtFilterState& filterState, const double& tEnd)
	{
		...
		while(filterState.t_<tEnd){
			...
			if(filterState.usePredictionMerge_){
				...
				if(r!=0) 
					std::cout << "Error during predictMerged: " << r << std::endl;
				logCountMerPre_++;
			}else{
				while(filterState.t_ < tNext && (predictionTimeline_.itMeas_ = predictionTimeline_.measMap_.upper_bound(filterState.t_)) != predictionTimeline_.measMap_.end()){
					double dt = std::min(predictionTimeline_.itMeas_->first, tNext) - filterState.t_;
					r = mPrediction_.performPrediction(filterState, predictionTimeline_.itMeas_->second, dt); // LWF: Prediction
					if(r != 0) 
						std::cout << "Error during performPrediction: " << r << std::endl;
					logCountRegPre_++;
				}
			}
